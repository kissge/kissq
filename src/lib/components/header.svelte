<script lang="ts">
	...
	$effect(() => {
		void [Game.activeRulesText, Game.currentState.defaultRule.limit];
		calcSize();
	});
</script>
		...
		<div>
			{#key Game.currentState.questionCount}
				<div in:fly={unlocked ? { x: -100 } : { x: 100 }}>
					...
					{#if Game.currentState.defaultRule.limit !== null}
						<small>
							/ <small>{Game.currentState.defaultRule.limit}</small>
						</small>
					{/if}
				</div>
			{/key}
